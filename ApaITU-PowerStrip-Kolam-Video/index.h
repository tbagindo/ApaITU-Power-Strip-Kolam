...
<html>
  ...
  <body>
    ...
    <p>
      <iframe width="200" height="122" src="https://www.youtube.com/embed/KCm-y2z40Z0" title="#1cApaITU PowerStrip Kolam" frameborder="0" allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen autoplay></iframe>
      <iframe width="200" height="122" src="https://www.youtube.com/embed/-XCEHxJLOTc" title="#2 ApaITU PowerStrip Kolam" frameborder="0" allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
    </p>
    <p>
    <i class="far fa-lightbulb" style="color:#00add6;"></i>
    <span class="relay-labels">Relay #1 -D7</span>
    <label class="switch"> 
      <input type="checkbox" onchange="relayButton('relay1')" id="relay1" @@RELAY1STATE@@>
      <span class="slider round"></span>
    </label>
    </p>
    <p>
    <i class="far fa-lightbulb" style="color:#00add6;"></i>
    <span class="relay-labels">Relay #2 -D8</span>
    <label class="switch">
      <input type="checkbox" onchange="relayButton('relay2')" id="relay2" @@RELAY2STATE@@>
      <span class="slider round"></span>
    </label>
    </p>
    <p>
      ...
      <span class="WiFi-labels" id="rssi">@@RSSI@@</span>
      <sup class="units">dbi</sup>
    </p>
    <p>
      <i class="fas fa-wifi" style="color:#00add6;"></i>
      <span class="WiFi-labels">SSID</span>
      <span class="WiFi-labels" id="ssid">@@SSID@@</span>
    </p>
    
  </body>
  <script>
  function relayButton(id){
    const button = document.getElementById(id);
    if(button.checked){
      var url = "/update?state=on&relay="+id;
    }else{
      var url = "/update?state=off&relay="+id;
    }
  ...
  setInterval(function  () {
    ...
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        ...
        document.getElementById("relay1").checked = inputChecked;
      }     
    };
    xhttp.open("GET", "/relay1State", true);
    xhttp.send();
  }, 1000 ) ;
  ...
  setInterval(function  () {
    ...
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        ...
        document.getElementById("relay2").checked = inputChecked;
      }     
    };
    xhttp.open("GET", "/relay2State", true);
    xhttp.send();
  }, 1000 ) ;
  </script>
</html>
